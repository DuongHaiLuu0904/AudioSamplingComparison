...
{
    public class WasapiSampler : BaseAudioSampler
    {
        public override string MethodName => "WASAPI";
        public override string MethodDescription => "Uses Windows Audio Session API (WASAPI) through NAudio's WasapiCapture. This provides lower latency and better performance than WaveIn methods, using a more modern Windows audio stack.";
        
        private bool useEventSync = true; // Whether to use event-driven or polling approach
        private bool isRecording = false;
        
        public WasapiSampler(bool useEventSync = true)
        {
        ...
        public override async Task<AudioData> CaptureAudioAsync(int durationSeconds)
        {
            ...
            try
            {
                // Use standard WaveIn as a fallback since WasapiCapture isn't available in this version
                var captureDevice = new WaveIn();
                
                var tcs = new TaskCompletionSource<bool>();
                
                // Configure the device for our sampling parameters
                captureDevice.WaveFormat = new WaveFormat(SampleRate, BitsPerSample, Channels);
                captureDevice.BufferMilliseconds = SamplingIntervalMs;
